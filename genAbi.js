...
const allContracts = _.map(_.without(
...
console.log('has contracts', allContracts, _.concat(rootContracts, glob.GlobSync('build/contracts/tokens/*.json').found));

function processContract(contract) {
  const builtPath = 'build/contracts/' + path.basename(contract.replace('.sol', '.json'));
  ...
  if (fs.existsSync(builtPath)) {
    fs.readFile(builtPath, (err, data) => {
      ...
      fs.writeFileSync(builtPath.replace('build/', 'artnonmous-subgraph/').replace('/contracts/', '/abis/'), JSON.stringify(dataJson['abi'], null, 2));
    });
  }
}
